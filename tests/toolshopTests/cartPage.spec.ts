...
test.describe("Cart page Tests", () => {
  ...
  test("check item appears in the cart page after adding to cart", async ({
    ...
    const itemName =
      (
        await toolShop.inventoryItemPage.inventoryItemName.textContent()
      )?.trim() || "";
  ...
  test("check checkout page opens from cart page", async ({
    toolShop,
    page,
  }) => {
    await toolShop.inventoryItemPage.addToCart();
    ...
    await expect(page.locator(".step-indicator").nth(1)).toHaveCSS(
      "background-color",
      "rgb(128, 128, 128)",
    );
  });

  test("check product catalog page opens from cart page", async ({
    toolShop,
    page,
  }) => {
    await toolShop.inventoryItemPage.addToCart();
    await toolShop.layout.openCart();
    await toolShop.cartPage.continueShoppingButton.waitFor({
      state: "visible",
    });
  ...
  test("check checkout page opens contain sign in form if user is not logged in", async ({
    toolShop,
    page,
  }) => {
    await toolShop.inventoryItemPage.addToCart();
  ...
  test("check return to cart from checkout page", async ({
    toolShop,
    page,
  }) => {
    await toolShop.inventoryItemPage.addToCart();
