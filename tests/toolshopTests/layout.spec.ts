...
test.describe("Layout Tests", () => {
  ...
  test("check home page opens from header by clicking Home link", async ({ page, toolShop }) => {
    await toolShop.layout.openContactPage();
    await expect(page).toHaveURL("https://practicesoftwaretesting.com/contact");
  ...
  test("check home page opens from header by clicking app logo", async ({ page, toolShop }) => {
    await toolShop.layout.openContactPage();
    await expect(page).toHaveURL("https://practicesoftwaretesting.com/contact");
...
test("check categories list in header dropdown", async ({ page, toolShop }) => {
    await toolShop.layout.openCategories();
    const categories = await page.locator("[aria-label='nav-categories'] .dropdown-item").allInnerTexts();
    expect(categories).toEqual(Object.keys(toolshopData.categories));
  });

  test("check category page opens from header dropdown", async ({ page, toolShop }) => {
    await toolShop.layout.openCategories();
    await page.locator("[aria-label='nav-categories'] .dropdown-item").nth(0).click();
  ...
  test("check language list in header dropdown", async ({ page, toolShop }) => {
    ...
    const languages = await page.locator("[aria-labelledby='language'] .dropdown-item").allInnerTexts();
  ...
  test("check policy page opens from footer", async ({ page, toolShop }) => {
    ...
    await expect(page).toHaveURL(
      "https://practicesoftwaretesting.com/privacy"
    );
  });
