...
test.describe("Inventory item page Tests", () => {
  ...
  test("product params are visible on product item page", async ({ toolShop }) => {             
      ...
      await expect(toolShop.inventoryItemPage.inventoryItemDescription).toBeVisible();
  ...
  test("check product quantity increased by 1", async ({ toolShop }) => {    
    ...
    test("check add to favorites is restricted for not signed in user", async ({ toolShop }) => {  
      await toolShop.inventoryItemPage.addToFavoritesButton.click();
      await expect(toolShop.layout.toastContainer).toBeVisible();
      await expect(toolShop.layout.toastContainer).toHaveText("Unauthorized, can not add product to your favorite list.");
    });
  });